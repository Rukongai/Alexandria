...
It is designed to run on a home server, NAS, or private server via Docker Compose. Alexandria is not offered as a hosted application and has no telemetry: you run it and own it. A deployment can still use managed infrastructure, including hosted PostgreSQL and S3-compatible object storage.
...
**Database and file storage are separate.** PostgreSQL stores Alexandria's metadata and application state. Model files and thumbnails live in managed local or S3-compatible storage. A hosted database therefore does not make locally stored models available to another machine; deployments that need host portability use S3-compatible storage as well. Alexandria still assumes one backend process because upload state and some rate limits are process-local.
...
The API uses a consistent `{ data, meta, errors }` envelope on every response without exception. Pagination uses cursors. All endpoints are documented in `docs/API.md`.
