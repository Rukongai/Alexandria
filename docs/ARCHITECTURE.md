...
**Owns:** `import_sessions` table CRUD — creating sessions, updating their status, detected metadata, and persisted review draft, listing active sessions, ownership validation.

**Does not own:** File extraction (FileProcessingService), ingestion pipeline (IngestionService), storage cleanup.

**Behavior:** `create` inserts a session in `scanning` status with a 24-hour `expiresAt`. `update` patches status and any combination of `detected`, `manifest`, `stagingPath`, `modelId`, `draftMetadata`, and `error`. Migration `0013_add_import_session_draft_metadata.sql` adds the nullable JSONB `draft_metadata` column. `updateDraftMetadata` shallow-merges draft fields while separately merging nested `metadata` and `options`; an explicit existing-collection choice removes a staged new-collection choice and vice versa. Assistant proposal validation accepts only owned, active-library sessions in `ready_for_review`, and apply locks referenced sessions in deterministic ID order before merging their drafts. `listActive` returns sessions in `scanning`, `ready_for_review`, `committing`, or `error` status for a given user and library. `getOwnedRow` fetches a session and throws `NOT_FOUND` if it doesn't exist or belongs to a different user. `toDto` maps the DB row to the `ImportSession` API shape (omitting `manifest` and `stagingPath`, but exposing `draftMetadata`). For list and detail reads, the service resolves a committing session's live progress through JobService. If the queue has not published valid structured progress or the lookup fails, the response remains available with a synthetic `queued`/0% value whose totals come from detected metadata when available. `commitProgress` is `null` for every non-committing status.
...
**Interface:** `IStorageService` exposes store, buffered and streaming retrieval, copy, delete, and existence checks over backend-independent logical keys. `store` accepts an optional observational callback with the bytes transferred for the current object; local and S3 implementations both report it, and callback failures never fail the underlying write. `LocalStorageService` is the default. `S3StorageService` is selected with `STORAGE_BACKEND=s3` and applies the optional `S3_PREFIX` when translating a logical key to an object key. All other services interact with this interface and do not construct filesystem paths, S3 requests, or public object URLs.
